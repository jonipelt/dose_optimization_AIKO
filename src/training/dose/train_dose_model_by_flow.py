...
METRICS_OUTPUT_PATH = "models/metrics/metrics_dose_flow.csv"  # Change if needed
...
df["basin_count"] = df["flow_rate"].apply(calculate_basin_count)
df["flow_class"] = df["flow_rate"].apply(classify_flow)
...
def train_model_for_flow_class(flow_class, df_sub):
    print(f"\nTraining dose model for flow class: {flow_class} ({len(df_sub)} rows)")

    features = ["raaka_sameus", "tuleva_virtaus", "tuleva_lampotila", "alku_pH", "basin_count"]
    target = "kemikaaliannos"
    ...
    model_filename = f"dose_model_flow_{flow_class}.pkl"
...
# Train model per class
for flow_class in df["flow_class"].unique():
